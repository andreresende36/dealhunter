...
class SupabaseClient:
    ...
    async def claim_for_enrichment(self, batch_size: int = 10) -> list[dict]:
        ...
        Seleciona produtos com enrichment_status='pending', atualiza para
        'in_progress' e retorna os dados necessários para o deep scrape.
        """
        try:
            ...
            # 2. Claim atômico (guard contra race condition)
            await (
            ...
            logger.debug("enrichment_claimed", count=len(ids))
            return result.data
    ...
    async def set_enrichment_status(
        ...
        data: dict = {"enrichment_status": status}
        if status == "failed":
    ...
    async def increment_enrichment_attempts(self, product_id: str) -> bool:
        ...
        try:
            ...
            current = result.data[0]["enrichment_attempts"] if result.data else 0
            await (
                self._db.table("products")
                .update({"enrichment_attempts": current + 1})
                .eq("id", product_id)
                .execute()
            )
            return True
    ...
    async def update_enriched_data(self, product_id: str, data: dict) -> bool:
        ...
        Args:
            ...
            data: Dict com campos enriquecidos (seller_name, seller_reputation,
                  sold_quantity, rating_stars, rating_count, etc.)
        """
        try:
            await (
                self._db.table("products")
            ...
            logger.debug("enriched_data_updated", product_id=product_id)
            return True
        except Exception as exc:
            raise SupabaseError(
    ...
    async def get_product_for_scoring(self, product_id: str) -> Optional[dict]:
        """Retorna dados completos de um produto para re-scoring após enriquecimento."""
        try:
            result = (
                await self._db.table("products")
                .select("*")
    ...
    async def reset_stale_claims(self, stale_minutes: int = 30) -> int:
        """
        Reseta produtos que ficaram 'in_progress' por muito tempo (crash recovery).
        Retorna o número de produtos resetados.
        """
        cutoff = (
        ...
        try:
            ...
            if count > 0:
                logger.info("stale_claims_reset", count=count)
            return count
        except Exception as exc:
            raise SupabaseError(
